...
RSpec.describe SocialMetaHelper, type: :helper do
  ...
  describe "#open_graph_tag" do
    ...
    context "when using a symbol for content" do
      ...
      it "generates a meta tag with the i18n content" do
        expect(subject).to match(%r{<meta property="og:site_name" content="Petitions - The Scottish Parliament" />})
      end
    end
    ...
    context "when using a image path for content" do
      ...
      it "generates a meta tag with the correct asset image url" do
        expect(subject).to match(%r{<meta property="og:image" content="https://petitions.parliament.scot/assets/os-social/opengraph-image.png" />})
      end
    end
  end
  ...
  describe "#twitter_card_tag" do
    context "when using a string for content" do
      subject do
        helper.twitter_card_tag("site", "@SP_Petitions")
      end

      it "generates a meta tag with the content" do
        expect(subject).to match(%r{<meta name="twitter:site" content="@SP_Petitions" />})
      end
    end
    ...
    context "when using a symbol for content" do
      ...
      it "generates a meta tag with the i18n content" do
        expect(subject).to match(%r{<meta name="twitter:title" content="Petitions - The Scottish Parliament" />})
      end
    end
    ...
    context "when using a image path for content" do
      ...
      it "generates a meta tag with the correct asset image url" do
        expect(subject).to match(%r{<meta name="twitter:image" content="https://petitions.parliament.scot/assets/os-social/opengraph-image.png" />})
      end
    end
  end
end
