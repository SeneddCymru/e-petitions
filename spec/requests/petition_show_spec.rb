...
RSpec.describe "API request to show a petition", type: :request, show_exceptions: true do
  ...
  describe "format" do
    it "responds to JSON" do
      get "/petitions/#{'PE%04d' % petition.pe_number_id}.json"
    ...
    it "does not respond to XML" do
      get "/petitions/#{'PE%04d' % petition.pe_number_id}.xml"
  ...
  describe "links" do
    ...
    it "returns a link to itself" do
      ...
      expect(response).to be_successful
      expect(links).to include("self" => "https://petitions.parliament.scot/petitions/#{'PE%04d' % petition.pe_number_id}.json")
    end
  end
