...
RSpec.describe "Content Security Policy header", type: :request do
  ...
  it "sets connect-src to 'self' https://apikeys.civiccomputing.com" do
    get "/"
    expect(subject).to match(/connect-src 'self' https:\/\/apikeys\.civiccomputing\.com/)
  end

  it "sets script-src to 'self' 'unsafe-inline' https://cc.cdn.civiccomputing.com" do
    get "/"
    expect(subject).to match(/script-src 'self' 'unsafe-inline' https:\/\/cc\.cdn\.civiccomputing\.com/)
  end
