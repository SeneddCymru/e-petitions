...
RSpec.describe "admin user persistence token", type: :request, csrf: false do
  let(:user_attributes) do
    {
      ...
      email: "admin@petitions.parliament.scot",
      password: "L3tme1n!",
  ...
  let(:login_params) do
    { email: "admin@petitions.parliament.scot", password: "L3tme1n!" }
  end
  ...
  def new_browser
    open_session do |s|
      s.reset!
      s.host! "moderate.petitions.parliament.scot"
      s.https!
    end
  end
  ...
  context "when a new session is created" do
    it "logs out existing sessions" do
      ...
      expect(s1.response.headers["Location"]).to eq("https://moderate.petitions.parliament.scot/admin")

      s2 = new_browser
      s2.post "/admin/user_sessions", params: { admin_user_session: login_params }

      expect(s2.response.status).to eq(302)
      expect(s2.response.headers["Location"]).to eq("https://moderate.petitions.parliament.scot/admin")

      s1.get("/admin")

      expect(s1.response.status).to eq(302)
      expect(s1.response.headers["Location"]).to eq("https://moderate.petitions.parliament.scot/admin/login")
    end
  end
  ...
  context "when a session is destroyed" do
    it "resets the persistence token" do
      ...
      expect(s2.response.status).to eq(302)
      expect(s2.response.headers["Location"]).to eq("https://moderate.petitions.parliament.scot/admin/login")
    end
  end

  context "when a session is stale" do
    before do
      host! "moderate.petitions.parliament.scot"
      https!
    end
