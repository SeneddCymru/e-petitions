        ...
        describe "for an open petition" do
          ...
          describe "with valid params" do
            it "redirects to the petition emails page" do
              do_post
              expect(response).to redirect_to "https://moderate.petitions.parliament.scot/admin/petitions/#{'PE%04d' % petition.pe_number_id}/emails"
            end
              ...
              it "should email out to the validated signees who have opted in when the delayed job runs" do
                perform_enqueued_jobs do
                  ...
                  expect(deliveries.map(&:to)).to eq([
                    ...
                    ["laura_1@example.com"],
                    ["laura_2@example.com"],
                    ["petitionscommittee@parliament.scot"]
                  ])
                end
              end
            end
    ...
    describe "DELETE /admin/petitions/:petition_id/emails/:id" do
      ...
      describe "for an open petition" do
        ...
        before do
          ...
          expect(moderated).to receive(:find_by_param!).with("#{petition.id}").and_return(petition)
