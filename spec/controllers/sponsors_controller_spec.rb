...
RSpec.describe SponsorsController, type: :controller do
  ...
  describe "GET /petitions/:petition_id/sponsors/new" do
    ...
    %w[open closed].each do |state|
      context "when the petition is #{state}" do
        ...
        it "redirects to the petition page" do
          expect(response).to redirect_to("/petitions/#{'PE%04d' % petition.pe_number_id}")
        end
      end
    end
    ...
    context "when the petition is rejected" do
      ...
      it "redirects to the petition page" do
        expect(response).to redirect_to("/petitions/#{'PP%04d' % petition.id}")
      end
    end
      ...
      context "when the petition is #{state}" do
        ...
        context "and has one remaining sponsor slot" do
          ...
          it "doesn't redirect to the petition moderation info page" do
            expect(response).not_to redirect_to("/petitions/#{'PP%04d' % petition.id}/moderation-info")
          end
        end
        ...
        context "and has reached the maximum number of sponsors" do
          ...
          it "redirects to the petition moderation info page" do
            expect(response).to redirect_to("/petitions/#{'PP%04d' % petition.id}/moderation-info")
          end
        end
      end
    end
      ...
      context "when the petition is #{state}" do
        ...
        context "and the signature is not a duplicate" do
          ...
          it "redirects to the thank you page" do
            expect(response).to redirect_to("/petitions/#{'PP%04d' % petition.id}/sponsors/thank-you?token=#{petition.sponsor_token}")
          end
    ...
    %w[open closed rejected].each do |state|
      context "when the petition is #{state}" do
        ...
        it "doesn't redirect to the petition page" do
          expect(response).not_to redirect_to("/petitions/#{'PE%04d' % petition.pe_number_id}}")
        end
    ...
    context "when the petition is validated" do
      ...
      context "and the signature has been validated more than 15 minutes ago" do
        ...
        it "redirects to the new sponsor page" do
          expect(response).to redirect_to("/petitions/#{'PP%04d' % petition.id}/sponsors/new?token=#{petition.sponsor_token}")
        end
      end
