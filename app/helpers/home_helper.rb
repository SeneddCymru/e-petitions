module HomeHelper
  class ActionedPetitionsDecorator
    ...
    def with_result
      {
        :open => actioned[:open],
        :referred => actioned[:referred],
        :with_debated_outcome => actioned[:with_debated_outcome]
      }
    end
  ...
  def fetch_trending_petitions(now, period, limit)
    ...
    Petition.trending(period.ago(now)..now, limit).pluck(:pe_number_id, :action, signature_count)
  end
